...
import { BsThreeDots, BsTextCenter } from "react-icons/bs";
...
interface MultiFilterRowProps<TData> extends MultiFilterProps<TData> {
	i: number;
...
export const MultiFilterRow = <TData,>({
	...
	const createQueryString = React.useCallback(
		(params: Record<string, string | number | null>) => {
			...
			// TODO: Needs to convert this to iterators
		...
		<div className="flex items-center space-x-2">
			...
			<DropdownMenu>
				...
				<DropdownMenuContent>
					...
					>
						<LuCopy className="mr-2 h-4 w-4" aria-hidden="true" />
						Duplicate
					</DropdownMenuItem>
				</DropdownMenuContent>
			</DropdownMenu>
...
interface MultiFilterProps<TData> {
	table: Table<TData>;
...
export const MultiFilter = <TData,>({
	table,
	allOptions,
	options,
	setSelectedOptions,
}: MultiFilterProps<TData>) => {
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				...
				>
					<BsTextCenter className="mr-2 h-3 w-3" aria-hidden="true" />
					{options.length} rule
				</Button>
			</PopoverTrigger>
